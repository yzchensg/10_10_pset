...
Original file is located at
    https://colab.research.google.com/drive/1bY11H9WNBGIqrWqpdV6KBX3wqEFtUWYg
"""
...
def checkmatrix(A):
  """
  Numerical method for checking a matrix for dependent equations
  ...
  It performs a couple of basic checks
    1. Rank
    2. If rank < N, then number of dependent equations is output
    3. Also dependent equation sets are listed
  """
  from scipy.linalg import null_space                       # make sure null space is active
  import numpy as np
  m, N = A.shape

  rankA = np.linalg.matrix_rank(A)                          # check matrix rank


  if rankA != N:
    # checking if there is a rank issue
    print("\n your A matrix has a problem because \n")
    print("its rank = ", rankA, "which is < N = ", N)

    # computing the null space of the transpose of A to get dependent equation set
    ns = null_space(A.transpose())

    # how many dependent equations
    ndepeq = np.shape(ns)[1]

    #Replace values below a threshold with zero for clear diagnostic
    ns[np.abs(ns) < 1e-10] = 0

  #
    print("\n\nnumber of dependent equations = ", ndepeq)

  # list out the equations in the dependent set
    for i in range(ndepeq):
      RAR = np.nonzero(ns[:,i-1])
      print("Python indices = ",RAR[0], " are a dependent set")
      print(ns[:,i-1])
    else:
      print("\n matrix A is appropriate to attempt solution \n")
  return